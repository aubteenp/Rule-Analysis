  ...
  ggplot() +
    ...
    geom_point(data = df_monthly, 
               aes(x = as.numeric(year) + (month_num-1)/12, 
                   y = .data[[yvar]]),
               colour = "gray40", size = 0.8, alpha = 0.5) +
...
make_plot_monthly <- function(df_monthly, yvar, ylab, title) {
  ...
  slope <- coef(model)[2]
  slope_lbl <- paste0("Monthly slope = ", round(slope*12, 3))  # Annualized for easier interpretation
  
  ggplot(df_monthly, aes(x = date_decimal, y = .data[[yvar]])) +
    geom_point(colour = pt_col, size = 1.5, alpha = 0.7) +  # Smaller points with some transparency
    geom_smooth(method = "loess", span = loess_span,
                se = FALSE, colour = smooth_col, linewidth = 1.2) +
    geom_smooth(method = "lm", se = FALSE, colour = trend_col,
                linetype = trend_lt, linewidth = 0.9) +
    annotate("text", x = min(df_monthly$date_decimal) + 2,  # Adjust position
...
rd_rules_dist_pre <- lm(rules ~ distance, data = filter(rd_data_all, treatment == 0))
rd_rules_full_pre <- lm(rules ~ distance + month + transition + D, data = filter(rd_data_all, treatment == 0))
...
rd_guidance_dist_pre <- lm(guidance ~ distance, data = filter(rd_data_all, treatment == 0))
rd_guidance_full_pre <- lm(guidance ~ distance + month + transition + D, data = filter(rd_data_all, treatment == 0))
...
rd_total_dist_pre <- lm(total_documents ~ distance, data = filter(rd_data_all, treatment == 0))
rd_total_full_pre <- lm(total_documents ~ distance + month + transition + D, data = filter(rd_data_all, treatment == 0))
...
rd_prop_dist_pre <- lm(proportion_rules ~ distance, data = filter(rd_data_all, treatment == 0))
rd_prop_full_pre <- lm(proportion_rules ~ distance + month + transition + D, data = filter(rd_data_all, treatment == 0))


# 2. Render tables ----------------------------------------------------------

# Rules table
stargazer(rd_rules_dist_all, rd_rules_full_all, rd_rules_dist_pre, rd_rules_full_pre,
          title = "RD Models for FDA Rules",
          column.labels = c("Distance-only (all)", "Full (all)", "Distance-only (pre)", "Full (pre)"),
...
# Guidance table
stargazer(rd_guidance_dist_all, rd_guidance_full_all, rd_guidance_dist_pre, rd_guidance_full_pre,
...
stargazer(rd_total_dist_all, rd_total_full_all, rd_total_dist_pre, rd_total_full_pre,
...
# Proportion table
stargazer(rd_prop_dist_all, rd_prop_full_all, rd_prop_dist_pre, rd_prop_full_pre,
